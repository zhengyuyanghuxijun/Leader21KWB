...
@implementation NewMyProgressView
...
@interface TextGridCell()
{
    UILabel *_readProgressLabel;
    NewMyProgressView *_progressControl;
...
- (void) initUI
{
    ...
    //阅读进度条
    _progressControl = [[NewMyProgressView alloc] initWithFrame:CGRectMake(15, 30, self.frame.size.width - 30, 6)];
...
//    [self didTap];
    [self.selectDelegate didSelectTextGridCell:button.tag forCell:self];
    self.isTest = NO;
}
...
- (void)updateFormData:(NSDictionary*)dic forIndex:(NSInteger)index
{
    ...
    self.bookCoverButton.tag = index;

...
-(DownloadEntity *)queryEntityByUrl:(NSString *)url
{
    ...
    if ([DE islogin]) {
        predicate = [NSPredicate predicateWithFormat:@"(userId == %@ || userId == %@) AND downloadUrl == %@", @(0), @(DE.me.userID.longLongValue), url];
    }
...
-(NSMutableArray *)getAllStoreDownloadTask
{
    ...
    if ([DE islogin]) {
        predicate = [NSPredicate predicateWithFormat:@"userId == %@ || userId == %@", @(0), @(DE.me.userID.longLongValue)];
    }
    ...
    {
        NSPredicate* pre = [NSPredicate predicateWithFormat:@"bookUrl == %@ AND ownerId == %lld", entity.downloadUrl, [DE currentUserId]];
...
-(void)insertDownloadTask:(DownloadItem *)item
{
    ...
    if(![self isExistDownloadTask:[item.url description]])
    {
        ...
        NSPredicate* pre = [NSPredicate predicateWithFormat:@"bookUrl == %@ AND ownerId == %lld", item.url, [DE currentUserId]];
        BookEntity* book = (BookEntity*)[CoreDataHelper getFirstObjectWithEntryName:@"BookEntity" withPredicate:pre];
        ...
        entity.isFree = @(book.priceF.floatValue <= 0.0f);
        if ([DE islogin] && !entity.isFree.boolValue) {
            entity.userId = DE.me.userID;
        }
        else {
            entity.userId = @(0);
        }

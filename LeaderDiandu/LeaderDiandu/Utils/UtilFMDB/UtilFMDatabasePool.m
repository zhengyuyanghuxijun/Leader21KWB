...
@interface UtilFMDatabasePool()

- (void)pushDatabaseBackInPool:(UtilFMDatabase*)db;
- (UtilFMDatabase*)db;

@end


@implementation UtilFMDatabasePool
...
+ (instancetype)databasePoolWithPath:(NSString*)aPath flags:(int)openFlags {
    return UtilFMDBReturnAutoreleased([[self alloc] initWithPath:aPath flags:openFlags]);
}
...
- (instancetype)initWithPath:(NSString*)aPath flags:(int)openFlags {
    ...
    if (self != nil) {
        ...
        _databaseInPool     = UtilFMDBReturnRetained([NSMutableArray array]);
        _databaseOutPool    = UtilFMDBReturnRetained([NSMutableArray array]);
...
- (void)dealloc {
    ...
    UtilFMDBRelease(_databaseInPool);
    UtilFMDBRelease(_databaseOutPool);
    
    if (_lockQueue) {
        UtilFMDBDispatchQueueRelease(_lockQueue);
        _lockQueue = 0x00;
    }
...
- (void)pushDatabaseBackInPool:(UtilFMDatabase*)db {
    ...
    [self executeLocked:^() {
        ...
        if ([_databaseInPool containsObject:db]) {
            [[NSException exceptionWithName:@"Database already in pool" reason:@"The UtilFMDatabase being put back into the pool is already present in the pool" userInfo:nil] raise];
        }
...
- (UtilFMDatabase*)db {
    ...
    [self executeLocked:^() {
        ...
        if (db) {
            ...
            db = [UtilFMDatabase databaseWithPath:_path];

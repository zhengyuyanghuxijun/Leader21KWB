...
#import "sqlite3.h"
#import "UtilFMResultSet.h"
#import "UtilFMDatabasePool.h"


#if ! __has_feature(objc_arc)
    #define UtilFMDBAutorelease(__v) ([__v autorelease]);
    #define UtilFMDBReturnAutoreleased UtilFMDBAutorelease

    #define UtilFMDBRetain(__v) ([__v retain]);
    #define UtilFMDBReturnRetained UtilFMDBRetain

    #define UtilFMDBRelease(__v) ([__v release]);

	#define UtilFMDBDispatchQueueRelease(__v) (dispatch_release(__v));
#else
    // -fobjc-arc
    #define UtilFMDBAutorelease(__v)
    #define UtilFMDBReturnAutoreleased(__v) (__v)

    #define UtilFMDBRetain(__v)
    #define UtilFMDBReturnRetained(__v) (__v)

    #define UtilFMDBRelease(__v)

// If OS_OBJECT_USE_OBJC=1, then the dispatch objects will be treated like ObjC objects
// and will participate in ARC.
// See the section on "Dispatch Queues and Automatic Reference Counting" in "Grand Central Dispatch (GCD) Reference" for details. 
    #if OS_OBJECT_USE_OBJC
        #define UtilFMDBDispatchQueueRelease(__v)
    #else
        #define UtilFMDBDispatchQueueRelease(__v) (dispatch_release(__v));
    #endif
#endif
...
/** A SQLite ([http://sqlite.org/](http://sqlite.org/)) Objective-C wrapper.
 ...
 ### Usage
 The three main classes in UtilFMDB are:

 - `UtilFMDatabase` - Represents a single SQLite database.  Used for executing SQL statements.
 - `<UtilFMResultSet>` - Represents the results of executing a query on an `UtilFMDatabase`.
 - `<UtilFMDatabaseQueue>` - If you want to perform queries and updates on multiple threads, you'll want to use this class.

 ### See also
 
 - `<UtilFMDatabasePool>` - A pool of `UtilFMDatabase` objects.
 - `<UtilFMStatement>` - A wrapper for `sqlite_stmt`.
 
 ### External links
 
 - [UtilFMDB on GitHub](https://github.com/ccgus/fmdb) including introductory documentation
 - [SQLite web site](http://sqlite.org/)
 - [UtilFMDB mailing list](http://groups.google.com/group/fmdb)
 - [SQLite FAQ](http://www.sqlite.org/faq.html)
 
 @warning Do not instantiate a single `UtilFMDatabase` object and use it across multiple threads. Instead, use `<UtilFMDatabaseQueue>`.

 */

@interface UtilFMDatabase : NSObject  {
...
/** Create a `UtilFMDatabase` object.
 
 An `UtilFMDatabase` is created with a path to a SQLite database file.  This path can be one of these three:

 1. A file system path.  The file does not have to exist on disk.  If it does not exist, it is created for you.
 2. An empty string (`@""`).  An empty database is created at a temporary location.  This database is deleted with the `UtilFMDatabase` connection is closed.
 3. `nil`.  An in-memory database is created.  This database will be destroyed with the `UtilFMDatabase` connection is closed.

 For example, to create/open a database in your Mac OS X `tmp` folder:

    UtilFMDatabase *db = [UtilFMDatabase databaseWithPath:@"/tmp/tmp.db"];

 Or, in iOS, you might open a database in the app's `Documents` directory:

    NSString *docsPath = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES)[0];
    NSString *dbPath   = [docsPath stringByAppendingPathComponent:@"test.db"];
    UtilFMDatabase *db     = [UtilFMDatabase databaseWithPath:dbPath];
 ...
 @return `UtilFMDatabase` object if successful; `nil` if failure.

 */

+ (instancetype)databaseWithPath:(NSString*)inPath;

/** Initialize a `UtilFMDatabase` object.
...
/** Execute select statement
 ...
 Executing queries returns an `<UtilFMResultSet>` object if successful, and `nil` upon failure.  Like executing updates, there is a variant that accepts an `NSError **` parameter.  Otherwise you should use the `<lastErrorMessage>` and `<lastErrorMessage>` methods to determine why a query failed.
 
 In order to iterate through the results of your query, you use a `while()` loop.  You also need to "step" (via `<[UtilFMResultSet next]>`) from one record to the other.
 ...
 @return A `<UtilFMResultSet>` for the result set upon success; `nil` upon failure. If failed, you can call `<lastError>`, `<lastErrorCode>`, or `<lastErrorMessage>` for diagnostic information regarding the failure.
 
 @see UtilFMResultSet
 @see [`UtilFMResultSet next`](<[UtilFMResultSet next]>)
 @see [`sqlite3_bind`](http://sqlite.org/c3ref/bind_blob.html)
 */

- (UtilFMResultSet *)executeQuery:(NSString*)sql, ...;
...
- (UtilFMResultSet *)executeQueryWithFormat:(NSString*)format, ... NS_FORMAT_FUNCTION(1,2);
...
- (UtilFMResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray *)arguments;
...
- (UtilFMResultSet *)executeQuery:(NSString *)sql withParameterDictionary:(NSDictionary *)arguments;


// Documentation forthcoming.
- (UtilFMResultSet *)executeQuery:(NSString*)sql withVAList: (va_list)args;
...
/** Adds SQL functions or aggregates or to redefine the behavior of existing SQL functions or aggregates.
 ...
 For example:
    ...
    [queue inDatabase:^(UtilFMDatabase *adb) {
        ...
        int rowCount = 0;
        UtilFMResultSet *ars = [adb executeQuery:@"select * from ftest where StringStartsWithH(foo)"];
        while ([ars next]) {
            rowCount++;
            NSLog(@"Does %@ start with 'h'?", [rs stringForColumnIndex:0]);
        }
        UtilFMDBQuickCheck(rowCount == 2);
    }];
...
/** Generate an `NSDateFormatter` that won't be broken by permutations of timezones or locales.
 ...
 Example:
    ...
    myDB.dateFormat = [UtilFMDatabase storeableDateFormat:@"yyyy-MM-dd HH:mm:ss"];
 ...
 @warning Note that `NSDateFormatter` is not thread-safe, so the formatter generated by this method should be assigned to only one UtilFMDB instance and should not be used for other purposes.
...
/** Set to a date formatter to use string dates with sqlite instead of the default UNIX timestamps.
 ...
 @param format Set to nil to use UNIX timestamps. Defaults to nil. Should be set using a formatter generated using UtilFMDatabase::storeableDateFormat.
 ...
 @warning Note there is no direct getter for the `NSDateFormatter`, and you should not use the formatter you pass to UtilFMDB for other purposes, as `NSDateFormatter` is not thread-safe.
 */
...
/** Objective-C wrapper for `sqlite3_stmt`
 ...
 This is a wrapper for a SQLite `sqlite3_stmt`. Generally when using UtilFMDB you will not need to interact directly with `UtilFMStatement`, but rather with `<UtilFMDatabase>` and `<UtilFMResultSet>` only.
...
@interface UtilFMStatement : NSObject {

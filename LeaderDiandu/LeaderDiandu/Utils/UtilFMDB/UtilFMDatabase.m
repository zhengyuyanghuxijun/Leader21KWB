...
- (UtilFMResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args;
- (BOOL)executeUpdate:(NSString*)sql error:(NSError**)outErr withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args;
@end

@implementation UtilFMDatabase
...
+ (instancetype)databaseWithPath:(NSString*)aPath {
    ...
    return UtilFMDBReturnAutoreleased([[self alloc] initWithPath:aPath]);
}
...
- (void)dealloc {
    [self close];
    UtilFMDBRelease(_openResultSets);
    UtilFMDBRelease(_cachedStatements);
    UtilFMDBRelease(_dateFormat);
    UtilFMDBRelease(_databasePath);
    UtilFMDBRelease(_openFunctions);
...
//    InfoLog("DB open db path:%s", [self sqlitePath]);

    int err = sqlite3_open([self sqlitePath], &_db );
    if(err != SQLITE_OK) {
//        ErrorLog("DB open failed!(%d) db path:%s", err, [self sqlitePath]);
//        DB_LOG(@"DB open failed!(%d) db path:%s", err, [self sqlitePath]);
...
// but for folks who don't bother noticing that the interface to UtilFMDatabase changed,
// we'll still implement the method so they don't get suprise crashes
- (int)busyRetryTimeout {
    NSLog(@"%s:%d", __FUNCTION__, __LINE__);
    NSLog(@"UtilFMDB: busyRetryTimeout no longer works, please use retryTimeout");
    return -1;
}

- (void)setBusyRetryTimeout:(int)i {
    NSLog(@"%s:%d", __FUNCTION__, __LINE__);
    NSLog(@"UtilFMDB: setBusyRetryTimeout does nothing, please use setRetryTimeout:");
}
...
- (void)closeOpenResultSets {
    ...
    NSSet *openSetCopy = UtilFMDBReturnAutoreleased([_openResultSets copy]);
...
- (void)resultSetDidClose:(UtilFMResultSet *)resultSet {
    NSValue *setValue = [NSValue valueWithNonretainedObject:resultSet];
    
    [_openResultSets removeObject:setValue];
}

- (UtilFMStatement*)cachedStatementForQuery:(NSString*)query {
    
    NSMutableSet* statements = [_cachedStatements objectForKey:query];
    
    return [[statements objectsPassingTest:^BOOL(UtilFMStatement* statement, BOOL *stop) {
...
- (void)setCachedStatement:(UtilFMStatement*)statement forQuery:(NSString*)query {
...
+ (NSDateFormatter *)storeableDateFormat:(NSString *)format {
    ...
    NSDateFormatter *result = UtilFMDBReturnAutoreleased([[NSDateFormatter alloc] init]);
    result.dateFormat = format;
    result.timeZone = [NSTimeZone timeZoneForSecondsFromGMT:0];
    result.locale = UtilFMDBReturnAutoreleased([[NSLocale alloc] initWithLocaleIdentifier:@"en_US"]);
    return result;
}
...
- (void)setDateFormat:(NSDateFormatter *)format {
    UtilFMDBAutorelease(_dateFormat);
    _dateFormat = UtilFMDBReturnRetained(format);
}
...
- (BOOL)goodConnection {
    ...
    UtilFMResultSet *rs = [self executeQuery:@"select name from sqlite_master where type='table'"];
...
- (void)warnInUse {
    NSLog(@"The UtilFMDatabase %@ is currently in use.", [self description]);
    
#ifndef NS_BLOCK_ASSERTIONS
    if (_crashOnErrors) {
        NSAssert1(false, @"The UtilFMDatabase %@ is currently in use.", self);
        abort();
    }
#endif
}
...
- (BOOL)databaseExists {
    ...
    if (!_db) {
        ...
        NSLog(@"The UtilFMDatabase %@ is not open.", [self description]);
        
    #ifndef NS_BLOCK_ASSERTIONS
        if (_crashOnErrors) {
            NSAssert1(false, @"The UtilFMDatabase %@ is not open.", self);
            abort();
        }
    #endif
//        ErrorLog("DB is NULL!");
...
- (NSError*)errorWithMessage:(NSString*)message {
    ...
    return [NSError errorWithDomain:@"UtilFMDatabase" code:sqlite3_errcode(_db) userInfo:errorMessage];    
}
...
//        ErrorLog("databaseExists is false!");
        return 0x00;
    }
    
    if (_isExecutingStatement) {
        [self warnInUse];
//        ErrorLog("databaseExists is _isExecutingStatement!");
        return 0x00;
    }
    ...
    UtilFMStatement *statement  = 0x00;
    UtilFMResultSet *rs         = 0x00;
    ...
    if (!pStmt) {
        ...
        if (SQLITE_OK != rc) {
            ...
            if (_logsErrors) {
//                ErrorLog("DB Error: %d \"%s\"", [self lastErrorCode], [[self lastErrorMessage] UTF8String]);
//                ErrorLog("DB Query: %s", [sql UTF8String]);
//                ErrorLog("DB Path: %s", [_databasePath UTF8String]);
#ifndef NS_BLOCK_ASSERTIONS
                if (_crashOnErrors) {
                    abort();
    ...
    UtilFMDBRetain(statement); // to balance the release below
    
    if (!statement) {
        statement = [[UtilFMStatement alloc] init];
    ...
    rs = [UtilFMResultSet resultSetWithStatement:statement usingParentDatabase:self];
    [rs setQuery:sql];
...
- (UtilFMResultSet *)executeQueryWithFormat:(NSString*)format, ... {
    va_list args;
...
- (BOOL)executeUpdate:(NSString*)sql error:(NSError**)outErr withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args {
    ...
    int rc                   = 0x00;
    sqlite3_stmt *pStmt      = 0x00;
    UtilFMStatement *cachedStmt  = 0x00;
    ...
    if (_shouldCacheStatements && !cachedStmt) {
        cachedStmt = [[UtilFMStatement alloc] init];
        
        [cachedStmt setStatement:pStmt];
        
        [self setCachedStatement:cachedStmt forQuery:sql];
        
        UtilFMDBRelease(cachedStmt);
    }
...
void UtilFMDBBlockSQLiteCallBackFunction(sqlite3_context *context, int argc, sqlite3_value **argv);
void UtilFMDBBlockSQLiteCallBackFunction(sqlite3_context *context, int argc, sqlite3_value **argv) {
#if ! __has_feature(objc_arc)
...
- (void)makeFunctionNamed:(NSString*)name maximumArguments:(int)count withBlock:(void (^)(sqlite3_context *context, int argc, sqlite3_value **argv))block {
    ...
    id b = UtilFMDBReturnAutoreleased([block copy]);
    
    [_openFunctions addObject:b];
    
    /* I tried adding custom functions to release the block when the connection is destroyed- but they seemed to never be called, so we use _openFunctions to store the values instead. */
#if ! __has_feature(objc_arc)
    sqlite3_create_function([self sqliteHandle], [name UTF8String], count, SQLITE_UTF8, (void*)b, &UtilFMDBBlockSQLiteCallBackFunction, 0x00, 0x00);
#else
    sqlite3_create_function([self sqliteHandle], [name UTF8String], count, SQLITE_UTF8, (__bridge void*)b, &UtilFMDBBlockSQLiteCallBackFunction, 0x00, 0x00);
#endif
}
...
- (NSString*)description {
    return [NSString stringWithFormat:@"%@ \nDB Path: %s\n", [super description], [self sqlitePath]];
}
...
@implementation UtilFMStatement

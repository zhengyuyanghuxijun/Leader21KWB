    ...
    for (NSInteger idx = (NSInteger)[downloaders count] - 1; idx >= 0; idx--)
    {
        ...
        if (aDownloader == downloader)
        {
            ...
            if (image)
            {
                ...
                if ([delegate respondsToSelector:@selector(webImageManager:didFinishDownloadWithImage:)])
                {
                    [delegate performSelector:@selector(webImageManager:didFinishDownloadWithImage:) withObject:self withObject:image];
                    isProcessed = TRUE;
                }
                if ([delegate respondsToSelector:@selector(webImageManager:didFinishDownloadWithImage:forURL:)])
                {
                    objc_msgSend(delegate, @selector(webImageManager:didFinishDownloadWithImage:forURL:), self, image, downloader.url);
                    isProcessed = TRUE;
                }
                if ([delegate respondsToSelector:@selector(webImageManager:didFinishDownloadWithImage:forURL:userInfo:)])
                {
                    NSDictionary *userInfo = [[downloadInfo objectAtIndex:uidx] objectForKey:@"userInfo"];
                    if ([userInfo isKindOfClass:NSNull.class])
                    {
                    ...
                    objc_msgSend(delegate, @selector(webImageManager:didFinishDownloadWithImage:forURL:userInfo:), self, image, downloader.url, userInfo);
                    isProcessed = TRUE;
                }

...
- (void)setMode:(LDProgressHUDMode)newMode {
...
- (void)setProgress:(float)newProgress {
    ...
    if (mode == LDProgressHUDModeDeterminate) {
		if ([NSThread isMainThread]) {
			[self updateProgress];
			[self setNeedsDisplay];
		} else {
...
- (void)updateProgress {
    [(LDRoundProgressView *)indicator setProgress:progress];
}
...
- (void)updateIndicators {
    ...
    if (mode == LDProgressHUDModeDeterminate) {
#if __has_feature(objc_arc)
        self.indicator = [[LDRoundProgressView alloc] init];
#else
        self.indicator = [[[LDRoundProgressView alloc] init] autorelease];
#endif
        [self addSubview:indicator];
}
    else if (mode == LDProgressHUDModeCustomView && self.customView != nil){
        self.indicator = self.customView;
        [self addSubview:indicator];
    } else if (mode == LDProgressHUDModeIndeterminate){
#if __has_feature(objc_arc)
        ...
        [self addSubview:indicator];
	} else if (mode == LDProgressHUDModeText) {
		self.indicator = nil;
	}
...
+ (LDProgressHUD *)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
    LDProgressHUD *hud = [[LDProgressHUD alloc] initWithView:view];
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
	...
	if (viewToRemove != nil) {
		LDProgressHUD *HUD = (LDProgressHUD *)viewToRemove;
		HUD.removeFromSuperViewOnHide = YES;
		[HUD hide:animated];
		return YES;
	} else {
...
- (id)initWithView:(UIView *)view {
	...
	if (!view) {
		[NSException raise:@"LDProgressHUDViewIsNillException" 
					format:@"The view used in the LDProgressHUD initializer is nil."];
	}
...
- (id)initWithFrame:(CGRect)frame {
	...
	if (self) {
        ...
        self.animationType = LDProgressHUDAnimationFade;
        self.mode = LDProgressHUDModeIndeterminate;
        self.labelText = nil;
...
- (void)layoutSubviews {
    ...
    if (!indicator) {
        self.height = indFrame.size.height ;
    }else
...
- (void)showUsingAnimation:(BOOL)animated {
    self.alpha = 0.0f;
    if (animated && animationType == LDProgressHUDAnimationZoom) {
    ...
    if (animated) {
        ...
        self.alpha = 1.0f;
        if (animationType == LDProgressHUDAnimationZoom) {
...
@implementation LDRoundProgressView
...
- (void)drawRect:(CGRect)rect {
    ...
    // Draw background
    CGContextSetRGBStrokeColor(context, 0.0f, 0.0f, 0.0f, 0.5f); // white
    CGContextSetRGBFillColor(context, 0.0f, 0.0f, 0.0f, 0.5f); // translucent white
    ...
    // Draw blank progress
//    CGFloat startAngle2 = (self.progress * 2 * (float)M_PI)- ((float)M_PI / 2) ;
//    CGFloat endAngle2 = 3 * ((float)M_PI / 2);
//    CGContextSetRGBFillColor(context, 0.76f, 0.76f, 0.76f, 0.8f); // white
//    CGContextMoveToPoint(context, center.x, center.y);
//    CGContextAddArc(context, center.x, center.y, radius, startAngle2, endAngle2, 0);
//    CGContextClosePath(context);
//    CGContextFillPath(context);
}
